...
resource "aws_s3_object" "bootstrap_kubernetes_master_install_postgres_client" {
  bucket  = aws_s3_bucket.bootstrap.id
  key     = "scripts/kubernetes-master/install-postgres-client.sh"
  content = <<-EOT
#!/bin/bash
set -euo pipefail
  ...
  "roles": {
    "kubernetes-master": {
      "selected_scripts": [
        ...
        "scripts/kubernetes-master/install-postgres-client.sh",
...
resource "aws_instance" "main" {
  ...
  iam_instance_profile = aws_iam_instance_profile.terrapilot_ec2_userdata.name
  ...
  depends_on = [
    ...
    aws_s3_object.bootstrap_kubernetes_master_install_postgres_client,
