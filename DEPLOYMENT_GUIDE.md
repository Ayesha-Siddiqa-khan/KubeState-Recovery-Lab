...
| 3 | install-postgres-client.sh | postgres-client | Kubernetes master / control plane only |
| 4 | install-nginx.sh | nginx | Kubernetes master / control plane only |
| 5 | common-setup.sh | kubernetes-common | All EC2 instances |
| 6 | control-plane-setup.sh | kubernetes-control-plane | Kubernetes master / control plane only |
| 7 | worker-join-setup.sh | kubernetes-worker-join | Kubernetes worker nodes only |
...
aws ssm get-parameter --name "/terrapilot/KubeState Recovery Lab API/dev/kubernetes/join-command/private" --with-decryption --region "us-east-1" --query Parameter.Value --output text
aws ssm delete-parameter --name "/terrapilot/KubeState Recovery Lab API/dev/kubernetes/join-command/private" --region "us-east-1"
```
