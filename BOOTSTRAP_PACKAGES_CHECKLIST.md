...
- `install-postgres-client.sh` (postgres-client)
...
| 3 | install-postgres-client.sh | postgres-client | Kubernetes master / control plane only | psql | - | - |
| 4 | install-nginx.sh | nginx | Kubernetes master / control plane only | nginx | - | nginx |
| 5 | common-setup.sh | kubernetes-common | All EC2 instances | containerd, kubelet, kubeadm, kubectl | - | containerd, kubelet |
| 6 | control-plane-setup.sh | kubernetes-control-plane | Kubernetes master / control plane only | kubeadm, kubectl | - | kubelet |
| 7 | worker-join-setup.sh | kubernetes-worker-join | Kubernetes worker nodes only | kubeadm, kubectl | - | kubelet |
...
**Required Commands:** psql
**Required Packages:** postgresql-client
**Notes:** PostgreSQL client only, not the server
...
```bash
for cmd in curl wget unzip git helm psql nginx containerd kubelet kubeadm kubectl; do
...
```bash
for svc in nginx containerd kubelet; do
...
```bash
dpkg -l | grep -E "curl|wget|unzip|git|ca-certificates|gnupg|helm|postgresql-client|nginx|containerd.io|kubelet|kubeadm|kubectl" || echo "No matching packages found"
```

### Package list check (Amazon Linux / RHEL)

```bash
rpm -qa | grep -E "curl|wget|unzip|git|ca-certificates|gnupg|helm|postgresql-client|nginx|containerd.io|kubelet|kubeadm|kubectl" || echo "No matching packages found"
```
